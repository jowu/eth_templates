...
\section*{Purpose}
This document describes the options and defaults provided by the eapsthesis class (file: eapsthesis.cls) used for BSc/MSc theses in the Department of Earth and Planetary Sciences (D-EAPS), ETH Zurich.
...
\section*{Supported class options and defaults}
...
\section*{Options explicitly disabled}
...
\section*{User-facing title-page commands}
The class provides convenience macros used on the title page:
\begin{description}
...
\section*{Behaviour differences when choosing \texttt{book} vs. \texttt{report}}
...
\section*{Page geometry and twoside}
...
\section*{Bibliography and lists}
The class loads \texttt{biblatex} (apa style, backend=biber) and redefines \verb|\printbibliography| so that the bibliography is added to the TOC (as a chapter in book mode, as a section in report mode). Similarly, list of figures/tables are added to the TOC at chapter or section level depending on mode.
...
\section*{Where to look in the class file}

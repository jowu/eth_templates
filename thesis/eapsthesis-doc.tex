...
\begin{description}
  ...
  \item[\texttt{nohyper}] \emph{Turns off} loading of the \texttt{hyperref} package. No hyperlinks for contents, references, 
    and bibliography will be generated. 
    By default, \texttt{hyperref} is loaded and used with the default package options for formatting. To override
    the formatting, use \verb|\hypersetup| in the preamble of your document.
\end{description}
...
The class requires several standard commands like \texttt{\textbackslash title} and \texttt{\textbackslash author}
to be present. Those are all standard and not documented here.

The class provides commands to fill in the title page:
\begin{description}
  \item[\texttt{\textbackslash studentid\{\}}] \textbf{Required}. Student ID printed on title page.
  \item[\texttt{\textbackslash mainadvisor[<university>]\{<name>\}\{<department>\}}] \textbf{Required}. Main supervisor 
    and department affiliation.
  \item[\texttt{\textbackslash coadvisor[<university>]\{<name>\}\{<department>\}}] \textbf{Required}. Additional 
    supervisor and department affiliation.
  \item[\texttt{\textbackslash examiner[<university>]\{<name>\}\{<department>\}}] \textbf{\texttt{mscthesis} only}. 
    Examiner and department affiliation.
  \item[\texttt{\textbackslash specialization\{\}}] \textbf{\texttt{mscthesis} only}. Master's program and 
    specialization text used for MSc title page.
\end{description}
...
All the advisor commands have arguments for name (and title), the department and optinally the university/institution
they are from. By default, the optional argument for university/institution is filled in as
\emph{Department of Earth and Planetary Sciences, ETH Zurich}. 
All arguments are passed through as plain text with no further formatting. 

\subsubsection*{Examples:}

Some examples are contained in the sample documents \texttt{deaps-thesis-sample-book.tex} and \texttt{deaps-thesis-sample-book.tex}.

\verb|\mainadvisor{Prof. Dr. One \textsc{Professor}}{Institute for Geology}| will be rendered as
\vspace{1ex}

\begin{tabular}{p{4cm}l}
Main supervisor & Prof. Dr. One \textsc{Professor} \\
    & Institute for Geology \\
    & Department of Earth and Planetary Sciences, ETH Zurich \\    
\end{tabular}
\vspace{1ex}

To change the university affiliation, use the optional first argument

\verb|\mainadvisor[NAGRA]{Dr. Some \textsc{Seismologist}}{Department for deep storage}| will be rendered as
\vspace{1ex}

\begin{tabular}{p{4cm}l}
Main supervisor & Dr. Some \textsc{Seismologist} \\
    & Department for deep storage \\
    & NAGRA \\    
\end{tabular}
...
\begin{itemize}
  ...
  \item \verb|xifthen| --- provides boolean conditionals and \verb|\newboolean|, \verb|\ifthenelse| used
   ...
   \item \texttt{hyperref} --- Unless turned off with the \texttt{nohyper} class option. 
\end{itemize}
...
must set that option. At the moment, this is hardcoded to \texttt{apa}, which is appropriate for 
D-EAPS theses. 
